 ...
 if (file_exists($image)) {
   ...
   if ($source_size !== false) {
     ...
     if ($source_aspect < $thumb_aspect) {
       $new_size = array($thumb_width, ($thumb_width / $source_size[0]) *
$source_size[1]);
       $source_pos = array(0, ($new_size[1] - $thumb_height) / 2);
     } else if ($source_aspect > $thumb_aspect) {
       $new_size = array(($thumb_width / $source_size[1]) * $source_size[0],
$thumb_height);
	...
	imagecopyresampled($thumb, $source, 0, 0, $source_pos[1], $source_pos[1], $new_size[0], $new_size[1], $source_size[0], $source_size[1]);
